...
use constant {
    ...
    TLV_IMGTYPE_BOOT_LOADER_2 => 3, # 2nd phase bootloader
};
...
our (@tlvname)  = qw(undef arch chip imgtype imgsubtype image timestamp rsasig);
...
sub writefile
{
    ...
    for my $t (@{$self->{tlv}}) {
        ...
        if($t->[0] == TLV_TYPE_DWORD) {
            ...
            $trailerdata .= pack("VVV", $type, $id, length($value));
            $trailerdata .= $value;
        } else {
