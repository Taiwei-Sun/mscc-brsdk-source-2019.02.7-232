...
define LINUX_POST_PATCH_FUNC
	rm -rf $(STAGING_DIR)/usr/src/linux
	mkdir -p $(STAGING_DIR)/usr/scr/linux
	( cd $(@D); \
		find . -path './include/*' -prune \
		    ...
		    -o -name '*.sh' \
		    -o -name '*.pl' \
		    -o -name '*.lds' \) -print | cpio -pd --preserve-modification-time $(STAGING_DIR)/usr/src/linux; \
		cp -a scripts include $(STAGING_DIR)/usr/src/linux; \
		( \
		    find arch -name include -type d -print | xargs -n1 -i: find : -type f \
		) | cpio -pd --preserve-modification-time $(STAGING_DIR)/usr/src/linux 2>/dev/null; \
	)
endef

LINUX_POST_PATCH_HOOKS += LINUX_POST_PATCH_FUNC

define LINUX_POST_INSTALL_IMAGES_FUNC_IMPL
	mv $(BINARIES_DIR)/$(LINUX_IMAGE_NAME) $(BINARIES_DIR)/mscc-linux-$(1).bin
	chmod +x $(BINARIES_DIR)/mscc-linux-$(1).bin
	xz --force --check=none --lzma2=preset=6e,dict=64KiB $(BINARIES_DIR)/mscc-linux-$(1).bin
	mkdir -p $(STAGING_DIR)/usr/src/mscc-linux-$(1)
	cp $(@D)/.config $(STAGING_DIR)/usr/src/mscc-linux-$(1)/.config
	cp $(@D)/Module.symvers $(STAGING_DIR)/usr/src/mscc-linux-$(1)
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(@D) mrproper
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(@D) O=$(STAGING_DIR)/usr/src/mscc-linux-$(1) prepare
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(@D) O=$(STAGING_DIR)/usr/src/mscc-linux-$(1) modules_prepare
	cp $(STAGING_DIR)/usr/src/mscc-linux-$(1)/.config $(@D)/.config
	$(call LINUX_FIXUP_DOT_CONFIG)
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(@D) prepare
	$(LINUX_MAKE_ENV) $(MAKE) $(LINUX_MAKE_FLAGS) -C $(@D) modules_prepare
	rm $(STAGING_DIR)/usr/src/mscc-linux-$(1)/source
	ln -fs ../linux $(STAGING_DIR)/usr/src/mscc-linux-$(1)/source
	sed -i -e 's@MAKEARGS := -C \(.*\)@MAKEARGS := -C ../linux@' $(STAGING_DIR)/usr/src/mscc-linux-$(1)/Makefile
endef

define LINUX_POST_INSTALL_IMAGES_FUNC
	$(call LINUX_POST_INSTALL_IMAGES_FUNC_IMPL,$(shell cut -d'/' -f2 <<<$(LINUX_KCONFIG_FILE)))
endef

LINUX_POST_INSTALL_IMAGES_HOOKS += LINUX_POST_INSTALL_IMAGES_FUNC
